...
from equipment.views.eqaccidentlist import Ui_Form
from equipment.controllers.equipmentcontroller import EquipmentController
from equipment.modules.accidentmodule import AccidentModule

import datetime


class EqAccidentListModule(QWidget, Ui_Form):
    def __init__(self, parent=None):
        super(EqAccidentListModule, self).__init__(parent)
        self.setupUi(self)
    ...
    def get_accident_note(self):
        ...
        res = self.EC.get_data(4, False, *VALUES_TUPLE_ACCIDENT, **key_dict)
        if not len(res):
            return
        for item in res:
            ...
            qtreeitem.setText(0, str(item['autoid']))
            qtreeitem.setText(
                1, item['occurdate'].strftime('%Y-%m-%d %H:%M') if type(
                    item['occurdate']) is datetime.datetime else ''
            )
            qtreeitem.setText(2, item['place'])
            qtreeitem.setText(3, item['kind'])
            qtreeitem.setText(4, item['injury'])
            qtreeitem.setText(5, item['brief'])
            qtreeitem.setText(6, item['reason'])
        for i in range(1, 7):
    ...
    def on_treeWidget_recordslist_customContextMenuRequested(self, pos):
        ...
        index = self.tabWidget.currentIndex()
        menu = QMenu()
        button1 = menu.addAction("增加事故记录")
        button2 = menu.addAction("修改事故记录")
        button3 = menu.addAction("删除事故记录")
        menu.addSeparator()
        ...
        if action == button1:
            if self.eqno != '':
                detail = AccidentModule(eqno=self.eqno, parent=self)
                detail.accepted.connect(self.get_accident_note)
                detail.show()
        elif action == button2:
            if current_item is not None and index == 0:
                id = int(current_item.text(0))
                detail = AccidentModule(autoid=id, parent=self)
                detail.accepted.connect(self.get_accident_note)
                detail.show()
        elif action == button3:
            if current_item is not None:
                id = int(current_item.text(0))
                condition = {'autoid': id}
                self.EC.delete_data(4, condition)
                self.get_accident_note()
        elif action == button4:
            if current_item is not None and index == 0:
                id = int(current_item.text(0))
                condition = {'autoid': id}
                kwargs = {'status': 1}
                self.EC.update_data(4, condition, **kwargs)
                self.get_accident_note()
...
VALUES_TUPLE_ACCIDENT = (
    'autoid', 'occurdate', 'place',  'kind', 'injury', 'brief', 'reason',
)
