...
from equipment.views.eqrepairlist import Ui_Form
from equipment.controllers.equipmentcontroller import EquipmentController
from equipment.modules.repairedmodule import RepairedModule

import datetime


class EqRepairListModule(QWidget, Ui_Form):
    def __init__(self, parent=None):
        super(EqRepairListModule, self).__init__(parent)
        self.setupUi(self)
    ...
    def get_repair_note(self):
        ...
        res = self.EC.get_data(3, False, *VALUES_TUPLE_REPAIR, **key_dict)
        if not len(res):
            return
        for item in res:
            ...
            qtreeitem.setText(
                2, item['repairerid'] + ' ' + item['repairername']
            )
            qtreeitem.setText(3, item['mainpoint'])
            qtreeitem.setText(4, item['partrepairing'])
    ...
    def on_treeWidget_recordslist_customContextMenuRequested(self, pos):
        ...
        index = self.tabWidget.currentIndex()
        menu = QMenu()
        button1 = menu.addAction("增加维修记录")
        button2 = menu.addAction("修改维修记录")
        button3 = menu.addAction("删除维修记录")
        menu.addSeparator()
        ...
        if action == button1:
            if self.eqno != '':
                detail = RepairedModule(eqno=self.eqno, parent=self)
                detail.accepted.connect(self.get_repair_note)
                detail.show()
        elif action == button2:
            if current_item is not None and index == 0:
                id = int(current_item.text(0))
                detail = RepairedModule(autoid=id, parent=self)
                detail.accepted.connect(self.get_repair_note)
                detail.show()
        elif action == button3:
...
VALUES_TUPLE_REPAIR = (
    'autoid', 'partrepairing', 'partreplacing', 'mainpoint', 'testrunning',
