...
from PyQt5.QtCore import Qt, QFile, QRect, pyqtSignal
...
class XMLReadWrite(QWidget, Ui_Form):
    changed_flat = pyqtSignal()
    ...
    def inputBox(self, element):
        ...
        widgetlabel.setText(
            self.set_vars(widgetlabel, element.attribute("Title")))
        # 宽度为0且内容不为空
        ...
        widgetlineedit.textEdited.connect(self.changed)
    ...
    def wireframe(self, element):
        ...
        qrect = QRect(self.current_X, self.current_Y, linewidth, lineheight)
    ...
    def checkBox(self, element):
        ...
        self.boxresize(widget)
        widget.setText(self.set_vars(widget,
                                     element.firstChildElement().attributeNode(
                                         "name").value()))
    ...
    def comboBox(self, element):
        ...
        widget.currentTextChanged.connect(self.changed_flat)
    ...
    def signBox(self, element):
        ...
        widget.signed.connect(self.changed_flat)
    ...
    def setupExpr(self, widgetAndElement):
        ...
        if expr_result:
            ...
            widget.setStyleSheet(
                "background-color: rgb(255, 255, 0);margin:2 2;")
        '''
        try:
            ...
            for item in set(sys_items):
                ...
                # 产品物料，分批次
                    ...
                    if len(var_list) == 0:
                        ...
                        '''
                        try:
                            exp = exp.replace(item, str(getattr(var_list[num], STUFF_DICT[vals[2:]])))
                        except IndexError:
                ...
                # 产品物料，不分批次
                    ...
                    try:
                        widget.varToWidget(item, (
                            0, getattr(var_list[num], STUFF_DICT[vals[3:]])))
                    except IndexError:
                        widget.varToWidget(item, (0, ''))
                # 检验报告类信息变量
        ...
        if self.autoid != 0:
            try:
                if kind == 0:
                    ...
                    vlist = (
                        'prodid', 'prodname', 'commonname', 'batchno', 'spec',
                        'package', 'medkind', 'planamount', 'mpunit',
                        'plantime', 'realamount', 'makedate', 'linename',
                        'workshopid', 'workshopname'
                    )
                    self.proddetail = pm.get_producingplan(
                        *vlist, autoid=self.autoid
                    )[0]
                elif kind == 1:
                    sm = StuffController()
